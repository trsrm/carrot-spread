# Notes

Organized editable notes and source-derived material live here.

`10-notes/` is allowed to change. Humans mostly edit these notes, and agents may also clean formatting, fix small errors, add metadata/backlinks, flag problems, or maintain the note when asked.

Do not replace original human notes with lossy LLM summaries. When processing inbox material, keep the captured wording, examples, facts, and context nearly as-is. Compiled LLM-maintained knowledge belongs in `20-knowledge/` or active project contexts in `30-projects/`.

Raw source-of-record capture lives in `00-inbox/`; do not rewrite it during normal processing unless the human explicitly asks.

## Provenance

Notes here should include `origin: personal | external | mixed` and `origin_note`. `origin` answers whose thought or authorship the material started from; `origin_note` gives the short explanation.

Examples: `origin: personal` for Taras's own memory or reflection, `origin: external` for a book/article/course/recipe digest, and `origin: mixed` when personal interpretation and outside material both matter.

## Naming

- Use short descriptive kebab-case filenames, for example `recipes/bbq-ribs.md`.
- Do not add leading date prefixes such as `2026-05-07-bbq-ribs.md`.
- Preserve capture/import dates in frontmatter, inbox processed markers, and `20-knowledge/log.md` instead of in note filenames.
- If two notes would have the same name, add a meaningful qualifier to the filename.

The starting note folders are practical defaults, not a fixed taxonomy. Agents may add new folders when a recurring note type or sensitive area needs a clearer home.
